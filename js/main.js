...
function fileSelected(){
	if (fileType.value === 'pdf') {
		...
		window.location.assign("../component/signup.html");
	} else {
...
function createList (index, parties, title, users, status, date, insight){
	...
	td1.innerHTML = `<select id="manage-btn">
							<option value="default">Download</option>
							<option value="doc">View</option>
							<option value="pdf">Rename</option>
							<option value="pdf">Edit</option>
							<option value="pdf">Delete</option>
						</select>`;
	td2.innerHTML = index;
